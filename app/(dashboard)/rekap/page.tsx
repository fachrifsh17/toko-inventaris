  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      if (!res.success || !(res as any).data) {
        ...
        const filteredPdfData = (res as any).data
          .map((item: any) => {
            if (!filterKategori) return item;
            const filteredDetails = item.detail_transaksi?.filter(
        ...
        const blob = await pdf(
          <RekapPdfDocument
            ...
            pengaturan={(res as any).pengaturan}
            filters={{
              type: typeParam,
              kategori: katParam,
      ...
      {showEdit && selected && (
        <PortalModal onClose={() => setShowEdit(false)}>
          ...
          <form key={selected.id} onSubmit={handleEditSubmit} className="p-6 space-y-4">
            ...
            <div>
              <label htmlFor="edit-tanggal" className="block text-sm font-medium text-slate-700 mb-1.5">Tanggal Transaksi</label>
              <input
                id="edit-tanggal"
                type="date"
                name="tanggal"
            ...
            {isKeluar && (
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="edit-biaya-lain" className="block text-sm font-medium text-slate-700 mb-1.5">Biaya Lain-lain</label>
                    <input
                      id="edit-biaya-lain"
                      type="text"
                      inputMode="numeric"
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex justify-between items-center text-sm">
                  ...
                  <span className="text-lg font-black text-pink-600">{formatRupiah(kembalianHitung)}</span>
                </div>
              </>
            )}
