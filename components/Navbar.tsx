...
import { getUnpaidTransactionsCount } from "@/actions/dashboard";
...
export default function Navbar({
  ...
  useEffect(() => {
    ...
    getUnpaidTransactionsCount().then((result) => {
      if (result.success && typeof result.count === "number") {
        setUnpaidCount(result.count);
      }
    });
  }, []);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {unpaidCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-rose-500 text-white rounded-full ring-2 ring-white flex items-center justify-center text-[10px] font-black group-hover:scale-110 transition-transform">
                {unpaidCount}
              </span>
            )}
          </button>
