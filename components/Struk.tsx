    ...
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 mx-auto font-mono text-[7.5pt] w-[190px] py-[8px] px-[6px] leading-[1.25] text-black">
      ...
      <div>
        ...
        <div className="flex justify-between text-[10pt] font-bold">
          <span>TOTAL</span><span>{idr(tHarga)}</span>
        </div>
      </div>
      {data.keterangan && (
...
<hr class="dv">
<div>
 ${data.detail_transaksi?.map((dt:any)=>`<div class="it"><div class="in">${dt.produk?.nama_produk??"-"}</div><div class="idt"><span>${dt.jumlah} x ${idr(dt.harga_jual_real)}</span></div><div class="is">${idr(dt.jumlah*dt.harga_jual_real)}</div></div>`).join("")}
</div>
<div class="ts">
<div class="tr"><span>Total Item</span><span>${totalItem} Pcs</span></div>
<hr class="dv">
<div class="tr gt"><span>TOTAL</span><span>${idr(totalHarga)}</span></div>
</div>
